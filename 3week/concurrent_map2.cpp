...
public:
	...
	WriteAccess operator[](const K& key)
	{
		...
		return  { lock_guard<mutex>(maps[index].m), maps[index].map[key] };
	};

	ReadAccess At(const K& key) const
	{
		auto index = getIndex(key);
		return  { lock_guard<mutex>(maps[index].m), maps[index].map.at(key) };
	};

	bool Has(const K& key) const
	{
		auto map = maps[getIndex(key)].map;
	...
	MapType BuildOrdinaryMap() const
	{
		...
		for (size_t count = 0; count < maps.size(); ++count)
		{
			lock_guard<mutex> guard(maps[count].m);
			allMaps.insert(begin(maps[count].map), end(maps[count].map));
		}
