...
struct AddressHasher {
	...
	{
		size_t hCity = shash(address.city);
		size_t hStreet = shash(address.street);
		size_t hBuilding = ihash(address.building);

		//Ax^2 + Bx + C
		size_t x = 37;
		return hCity * x * x + hStreet * x + hBuilding;
	}
...
struct PersonHasher {
	...
	{
		size_t hName = shash(person.name);
		size_t hHeight = ihash(person.height);
		size_t hWeight= dhash(person.weight);
		size_t hAddress = ahash(person.address);

		//Ax^3 + Bx^2 + Cx + D
		size_t x = 40;
		return hName * x * x * x + hHeight * x * x + hWeight * x + hAddress;
	}
