...
Node* Next(Node* me) 
{
    if (!me->left && !me->right && !me->parent) return nullptr;

    //(A) у текущей вершины есть правое поддерево
    if (me->right)
    {
        auto newNode = me->right;
    ...
    //(Б) у текущей вершины нет правого поддерева, а она сама является левым ребёнком у её родителя
    if (me->right == nullptr && me->parent->left == me)
    {
        return me->parent;
    }

    //(В) у текущей вершины нет правого поддерева, а она сама является правым ребёнком у её родителя.
    if (me->right == nullptr && me->parent->right == me)
    {
        auto newNode = me->parent;

        while(newNode)
        {
            if (newNode->value > me->value) break;
